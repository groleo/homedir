...
/**
 * Convert a morse string into an array of millisecond timings.
 * With the Farnsworth method, the morse characters are
 * played at one speed and the spaces between characters
 * at a slower speed.
 * 
 * morse - the morse code string
 * wpm - the speed in words per minute ("PARIS " as one word)
...
function morse2timingsFarnsworth(morse, wpm, farnsworth) {	
    ...
    var r = wpm / farnsworth;
    return morse2timingsGeneral(morse, dit, 3*dit
                               , dit, 3*dit*r, 7*dit*r);
}
...
function morse2timingsGeneral(morse, dit, dah, ditSpace, charSpace, wordSpace) {
    ...
    // abort if there's something that couldn't be translated
    if (morse.indexOf('?') != -1) {
        return [];
    }
...
function playMorse(morse, wpm, fwpm, freq, volume) {
    ...
    if (timings.length > 0) {
        timings.push(1000);
        // add in a second of silence on the end otherwise
        // IE just carries on beeping
...
function makeSample(timings, freq) {
    var buf = [];
    var counterIncrementAmount =
                               Math.PI * 2 * freq / SAMPLE_RATE;
    var on = 1;
